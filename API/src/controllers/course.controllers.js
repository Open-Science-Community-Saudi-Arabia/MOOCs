...
exports.getCourseData = async (req, res, next) => {
    ...
    for (let i = 0; i < course.course_sections.length; i++) {
      ...
      if (curr_section.exercises) {
        for (let j = 0; j < curr_section.exercises.length; j++) {
          ...
          const exercise_report = await ExerciseReport.findOne({
  ...
  if (req.user && course.enrolled_users.includes(req.user?.id)) {
    const course_report = await CourseReport.findOne({
      course: course._id,
      user: req.user.id,
    });
    if (course_report) {
...
exports.uploadVideo = async (req, res, next) => {
  ...
  let course_section = await CourseSection.findById(course_section_id).populate(
    "course"
  );
  if (!course_section) {
  ...
  if (course_section.course._id.toString() !== course_id) {
    return next(
      new ForbiddenError(
        "Course section does not belong to the course provided"
      )
    );
  }
...
exports.getCourseVideos = async (req, res, next) => {
  ...
  course_videos.videos = course_videos.videos.filter(
