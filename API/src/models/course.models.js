...
exerciseReportSchema.pre("save", async function (next) {
  ...
  if (this.isModified("best_score")) {
    // get the exercise
    const exercise = await Exercise.findById(this.exercise).populate(
      "questions"
    );

    this.percentage_passed =
      (this.best_score / exercise.questions.length) * 100;
  }
  next();
});
...
const DownloadableResource = mongoose.model(
